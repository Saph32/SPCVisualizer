...
alwaysinline uint8 SMP::ram_read(uint16 addr) {
  ...
  return g_oSoundModule.ReadRam(addr);
}
...
alwaysinline void SMP::ram_write(uint16 addr, uint8 data) {
  //writes to $ffc0-$ffff always go to apuram, even if the iplrom is enabled
    if (status.ram_writable && !status.ram_disable) g_oSoundModule.WriteRam(addr, data);
}

uint8 SMP::port_read(uint2 port) const {
  return g_oSoundModule.ReadRam(0xf4 + port);
}

void SMP::port_write(uint2 port, uint8 data) {
    g_oSoundModule.WriteRam(0xf4 + port, data);
}
...
uint8 SMP::op_busread(uint16 addr) {
  ...
  case 0xf3:  //DSPDATA
    ...
    return g_oSoundModule.ReadDspReg(status.dsp_addr & 0x7f);
  ...
  case 0xf7:  //CPUIO3
    //synchronize_cpu();
    //return 0;//cpu.port_read(addr);
      return port_read(addr & 0x3);
...
void SMP::op_buswrite(uint16 addr, uint8 data) {
  ...
  case 0xf3:  //DSPDATA
    ...
    g_oSoundModule.WriteDspReg(status.dsp_addr & 0x7f, data);
    break;
