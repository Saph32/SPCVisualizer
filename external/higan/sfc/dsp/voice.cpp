...
void DSP::voice_2(voice_t& v) {
    ...
    uint8 lo = g_oSoundModule.ReadRam((uint16)(addr + 0));
    uint8 hi = g_oSoundModule.ReadRam((uint16)(addr + 1));
...
void DSP::voice_3b(voice_t& v) {
    state.t_brr_byte = g_oSoundModule.ReadRam((uint16)(v.brr_addr + v.brr_offset));
    state.t_brr_header = g_oSoundModule.ReadRam((uint16)(v.brr_addr));
}
